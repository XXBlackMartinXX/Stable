...
class ObjWriter:
    ...
    def box(self, x0, y0, x1, y1, z0, z1, name, wall_mtl, roof_mtl="roof_metal", roof=True):
    ...
    def slab(self, x0, y0, x1, y1, z, name, mtl):
        self.lines.append(f"o {name}")
...
def main():
    w = ObjWriter()
    # Site ground pad
    w.slab(0, 0, SITE_WIDTH, SITE_DEPTH, -0.02, "site_ground_pad", "ground_pad")
    ...
    for s in spaces:
        ...
        if h <= 0.001:
            w.slab(x0, y0, x1, y1, 0.0, name, mtl)
        else:
            g = 0.06  # reveal gap between adjacent volumes
            w.box(x0 + g, y0 + g, x1 - g, y1 - g, 0.0, h, name, mtl)
